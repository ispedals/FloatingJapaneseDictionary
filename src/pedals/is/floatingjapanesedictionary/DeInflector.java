...
public class DeInflector {
	...
	static {
		...
		for (String line : stringRules.split("\n")) {
			...
			DeInflectorRuleEntry entry = new DeInflectorRuleEntry(
		...
		do {
			...
			for (int j = 0; j < rules.size(); ++j) {
				...
				if (ruleGroup.fromLen <= wordLen) {
					final String end = word.substring(word.length()
							- ruleGroup.fromLen);
					...
					for (int k = 0; k < ruleGroup.size(); ++k) {
						...
						if (((type & rule.type) > 0) && (end.equals(rule.from))) {
							final String newWord = word.substring(0,
									word.length() - rule.from.length())
									+ rule.to;
