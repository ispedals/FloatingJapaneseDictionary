...
public class DictionaryEntries extends ArrayList<DictionaryEntry>{
	...
	public static DictionaryEntries fromJSON(String jsonString){
		...
		try {
			...
			e.printStackTrace();
			throw new IllegalStateException("DictionaryEntries JSON string not unserializable");
		}
		...
		for(int i=0; i< entries.length(); i++){
			...
			try {
				...
				e.printStackTrace();
				throw new IllegalStateException("DictionaryEntry JSON string not unserializable");
			}
			coll.add(entry);
		}
